...
public enum IngredientName {
    ...
    PUMPKIN,
    TURNIP,
    MILK // Tambahkan ini jika belum ada
};
...
public class Ingredient : Object {
    ...
    // === [Mixer Related] ===
    public bool sendToMixer = false;
    public bool autoSendToMixer = false; // aktifkan ini kalau ingin langsung ke mixer saat spawn
    private bool alreadySentToMixer = false;
    ...
    public void Start() {
        ...
        // Jika autoSend aktif
        if (autoSendToMixer && !alreadySentToMixer) {
            SendToMixer();
        }
    }
    ...
    // === [MIXER SENDING] ===
    public void SendToMixer() {
        if (!alreadySentToMixer) {
            sendToMixer = true;
            alreadySentToMixer = true;

            // Tambahkan efek atau log
            Debug.Log($"{ingredientName} dikirim ke mixer!");

            // Misalnya: ubah posisi ke mixer (opsional)
            // transform.position = new Vector3(0, 1, 0);

            // Tambahkan animasi, partikel, suara, dll
        }
    }
}
